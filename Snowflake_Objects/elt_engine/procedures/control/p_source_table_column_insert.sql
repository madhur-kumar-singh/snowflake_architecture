...
BEGIN
    ...
    ELSEIF(:table_id IS NULL) THEN
        RAISE invalid_table;
    ELSEIF(EXISTS (SELECT source_table_column_id FROM elt_engine.control.source_table_columm WHERE (table_id = :table_id) AND (target_column_name = :target_column_name))) THEN
